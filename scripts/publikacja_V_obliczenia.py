...
print(f"ε_CP^res (asymetria CP) = {eps_CP_res}")
...
print(f"  2. Złamanie CP:       ε_CP = {eps_CP_res} ✓")
...
# =================================================================
print("\n" + "="*65)
print(" 2. RGE UNIFIKACJA SPRZĘŻEŃ")
print("="*65)
...
print(f"  -> Spin(10) daje NISKĄ skalę (10^11 vs 10^16)")
...
print(f"\n  τ_p (z M_GUT=10^11 GeV) ~ {tau_p_Spin10_naive:.2e} lat")
print(f"  τ_p (z M_GUT=10^16 GeV) ~ {tau_p_standard:.2e} lat")
print(f"  SK limit: > 1.6e34 lat")
...
print(f"f_a (skala PQ)          = {f_a_GeV:.2e} GeV")
...
# Detektory
print(f"\nDetektory:")
print(f"  ADMX/HAYSTAC (μeV):    poza zasięgiem (za ciężkie)")
...
print(f"  Źródło: 45 pól gauge Spin(10)")
print(f"  Detekcja: 14.5σ w CMB-S4 (2035)")
print(f"  Kształt: 70% equilateral + 30% local")

print("\n[SYGNATURA 2] B_TTB ≠ 0")
print(f"  Źródło: CP-łamanie z torsji chiralnej (δ_CP = {delta_CP})")
print(f"  Detekcja: polaryzacja B-mode w LiteBIRD")
print(f"  Unikalność: B_TTB = 0 we wszystkich innych modelach (CP zachowane)")

print("\n[SYGNATURA 3] m_a = 28 neV")
print(f"  Źródło: f_a = M_GUT (naturalna skala PQ)")
print(f"  Detekcja: CASPEr (2028)")
print(f"  Konsekwencja: kompletna ciemna materia Ω_a h² = 0.12")

# =================================================================
#  7. PORÓWNANIE 5 PUBLIKACJI
# =================================================================
print("\n" + "="*72)
print(" PORÓWNANIE 5 PUBLIKACJI - GŁÓWNE WYNIKI")
print("="*72)
print(f"{'Publ.':<10} | {'Temat':<35} | {'Kluczowy wynik'}")
print("-"*72)
...
print(f"{'LQG':<28} | {'✗':<11} | {'brak':<10} | {'QG':<8} | średnia")
print(f"{'SU(5) GUT':<28} | {'częśc.':<11} | {'ręczne':<10} | {'decay':<8} | wysoka")
print(f"{'SO(10) fenom.':<28} | {'✓':<11} | {'ręczne':<10} | {'średnio':<8} | średnia")
print(f"{'**Spin(10) sieciowy**':<28} | {'✓':<11} | {'**emerg.**':<10} | {'**25+**':<8} | **wysoka**")
print(f"{'M-theory':<28} | {'✓':<11} | {'?':<10} | {'landscape':<8} | niska")
print(f"{'Asymptotic Safety':<28} | {'✗':<11} | {'brak':<10} | {'UV fp':<8} | średnia")
print("="*72)
